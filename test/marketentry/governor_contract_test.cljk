  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Fabrication SRL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Fabrication SRL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest interdiction-list-listed-is-held-and-unoverridable
  (testing "interdiction-list membership -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:interdiction-list-listed} (-> (store/ledger db) last :basis)))
...
(deftest idno-unverified-is-held-and-unoverridable
  (testing "unverified IDNO (Numărul de identificare de stat) when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:idno-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
