(ns marketentry.marketentryllm
  ...
  the Moldova (MDA) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "idno-verified?=" (:idno-verified? e)
                        " on-interdiction-list?=" (:on-interdiction-list? e)
