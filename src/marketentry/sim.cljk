...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MDA, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up interdiction-list membership) ==")
    ...
    (println "== filing/submit eng-4 (interdiction-list membership -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up idno-unverified) ==")
    ...
    (println "== filing/submit eng-5 (idno-unverified -> HARD hold) ==")
