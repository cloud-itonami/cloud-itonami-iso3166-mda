(ns marketentry.store
  "SSoT for the Moldova (MDA) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:on-interdiction-list?` / `:requires-idno?` / `:idno-verified?` are
  ground truth for the flagship and conditional governor checks."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Fabrication SRL" :portal "MTender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-interdiction-list? false
             :requires-idno? true :idno-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MDA" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "MTender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-interdiction-list? false
             :requires-idno? true :idno-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems SRL" :portal "MTender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "MDA" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export SRL" :portal "MTender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-interdiction-list? true
             :requires-idno? true :idno-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MDA" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics SRL" :portal "MTender"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-interdiction-list? false
             :requires-idno? true :idno-verified? false
             :drafted? false :submitted? false
             :jurisdiction "MDA" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech SRL" :portal "MTender"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               on-interdiction-list?
                               requires-idno? idno-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? on-interdiction-list?)         (assoc :engagement/on-interdiction-list? on-interdiction-list?)
    (some? requires-idno?)                (assoc :engagement/requires-idno? requires-idno?)
    (some? idno-verified?)                (assoc :engagement/idno-verified? idno-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/on-interdiction-list?
   :engagement/requires-idno? :engagement/idno-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :on-interdiction-list? (boolean (:engagement/on-interdiction-list? m))
     :requires-idno? (boolean (:engagement/requires-idno? m))
     :idno-verified? (boolean (:engagement/idno-verified? m))
